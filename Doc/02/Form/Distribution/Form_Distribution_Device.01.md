...
xlist=<ol><li>{Def.Sec}</li><li>{Relate.Appoint.Sec}</li><li>{Product.Sec}</li><li>{Relate.Exclusive.Sec}</li><li>{BuySell.Minimum.Sec}</li><li>{Duty;Rep.P2.Sec}</li><li>{Price.Sec}</li><li>{Order.Sec}</li><li>{Ship.Sec}</li><li>{Pay;Secure.Sec}</li><li>{Duty.P1.Sec}</li><li>{Rep.P1.Product.Sec}</li><li>{IP.Sec}</li><li>{Conf.Sec}</li><li>{Indemn.Sec}</li><li>{Term.Sec}</li><li>{Remedy.Liab.Limit.Sec}</li><li>{Dispute.Sec}</li><li>{Misc.Sec}</li></ol>
...
Duty;Rep.P2.Sec=<b>Additional Duties, Responsibilities and Warranties of {P2.the}</b>.<ol><li>{Resale.Effort.Sales;Marketing.Sec}</li><li>{Resale.Limit.Customer.Sec}</li><li>{Duty.P2.BusinessStandards.Sec}</li><li>{Resale.Report.Sec}</li><li>{Resale.Advertise;Marketing.Costs;Guidelines.Sec}</li><li>{Resale.Service.Sec}</li><li>{Resale.Personnel.Sec}</li><li>{Resale.Manager.Sec}</li><li>{Resale.Traceability.Sec}</li><li>{Duty.P2.Permit.Sec}</li><li>{Duty.P2.Import.Sec}</li><li>{Resale.Customer.Inquiry.Sec}</li></ol>

Price.Sec=<b>Pricing and Discounts</b>.<ol><li>{Price.Agreed.Sec}</li><li>{Price.Promotion.Sec}</li><li>{Price.Change.Sec}</li><li>{Price.Tax.Sec}</li><li>{Price.Insurance;Freight.Sec}</li></ol>

Order.Sec=<b>Submission and Confirmation of Purchase Orders and Changes</b>.<ol><li>{Order.Submit.Sec}</li><li>{Order.Confirm.Sec}</li><li>{Order.Change.Sec}</li><li>{Order.Reschedule;Expedite.Sec}</li><li>{Order.Cancel.Sec}</li></ol>

Ship.Sec=<b>Shipment Terms</b>.<ol><li>{Ship.Base.Sec}</li><li>{Ship.Partial.Sec}</li><li>{Ship.Delay.Sec}</li></ol>

Pay;Secure.Sec=<b>Payment and Security Terms</b>.<ol><li>{Pay.Base.Sec}</li><li>{Pay.Currency.Sec}</li><li>{Pay.Secure.Sec}</li><li>{BuySell.Return.Sec}</li></ol>

Duty.P1.Sec=<b>Additional Responsibilities and Obligations of {P1.the}</b>.<ol><li>{Duty.P1.BuySell.Perform.Sec}</li><li>{Duty.P1.Train.Sec}</li><li>{Duty.P1.Support.Sec}</li><li>{Duty.P1.Promotion.Sec}</li><li>{Duty.P1.USExportLaw.Sec}</li></ol>

Rep.P1.Product.Sec=<b>{Product_Warranty}</b>.<ol><li>{Rep.Product.Base.Sec}</li><li>{Rep.Product.Disclaim.Sec}</li><li>{Rep.Product.P2.Modify.Sec}</li></ol>

IP.Sec=<b>Intellectual Property</b>.<ol><li>{IP.TM;TradeName.Sec}</li><li>{IP.TM.License.Sec}</li><li>{IP.Product.Sec}</li></ol>

Conf.Sec=<b>Confidentiality</b>.<ol><li>{Conf.Def.Def}</li><li>{Conf.Impart.Sec}</li><li>{Conf.Except.Sec}</li><li>{Conf.Engage.Sec}</li><li>{Conf.Except.Compelled.Sec}</li><li>{Conf.Remedy.Return.Sec}</li><li>{Conf.Remedy.Injunct.Sec}</li></ol>

Indemn.Sec=<b>Indemnification</b>.<ol><li>{Indemn.IP.P1.Sec}</li><li>{Indemn.General.Sec}</li></ol>

Term.Sec=<b>Term and Termination</b>.<ol><li>{Term.Base.Sec}</li><li>{Term.Stop.Reciprocal.Sec}</li><li>{Term.After.Sec}</li><li>{Term.Remedy.Sec}</li></ol>

Dispute.Sec=<b>Alternative Dispute Resolution</b>.<ol><li>{Dispute.ADR.Sec}</li><li>{Dispute.Arb.Sec}</li></ol>

Misc.Sec=<b>General Provisions</b>.<ol><li>{Misc.Relate.Sec}</li><li>{Misc.Majeure.Sec}</li><li>{Misc.Entire.Sec}</li><li>{Amend.Sec}</li><li>{Misc.Performance_by_Manufacturer.Sec}</li><li>{Misc.Assign.Sec}</li><li>{Misc.Language.Sec}</li><li>{Dispute.Law.Sec}</li><li>{Misc.Sever.Sec}</li><li>{Misc.Waive.Sec}</li><li>{Misc.Heading.Sec}</li><li>{Misc.Counterpart.Sec}</li><li>{Misc.Notice.Sec}</li></ol>

/Note=This was originally a form form the firm Ropes & Gray, some years ago.   Euro;Med Distribution Agt Library  Been changed in many ways.  Give thanks to R&G but don't rely on the fact that it originated there. 
...
5.	Additional Duties, Responsibilities and Warranties of  {P2.the}.
...
Price.Promotion.Sec=<b>Promotional {Products}</b>.  In the first {Contract_Year},  {P2.the} may purchase up to ______ (_) {Products} in aggregate at a discount of fifty percent (50%) off the prices set forth in the then-current Exhibit A for  {P2.the}&rsquo;s use in performing {Product} demonstrations and promotions (“{Promotional_Units}&rdquo;).  Any {Promotional_Units} purchased through the {Initial_Stocking_Order} shall be counted as {Promotional_Units} purchased in the first {Contract_Year}.

Price.Change.Sec=<b>Price Changes</b>. {P1.The} shall notify  {P2.the} at least sixty (60) days before any new pricing will become effective. {P1.The} shall attach to Exhibit A a supplement setting forth the new prices and the effective dates of each price change.  Any {Product} delivered by {P1.the} to {Carrier} for shipment to  {P2.the} after the effective date of a price change shall be invoiced at the price in effect at the time the {Order} was confirmed by {P1.the} in accordance with the terms of {this_Agreement}.
...
Ship.Base.Sec=<b>Shipment</b>. {P1.The} agrees to deliver the {Products} to the {Carrier} (as defined below) for shipment to  {P2.the}.  Title and risk for loss or damage shall pass to  {P2.the} upon delivery of {Products} to the {Carrier} for shipment to  {P2.the} and any loss or damage thereafter shall not relieve  {P2.the} of any obligation hereunder. {P2.The} shall be responsible for selecting and making all arrangements for a carrier (the “{Carrier}&rdquo;) to ship the {Products} from {P1.the}&rsquo;s warehousing facility, which may be located in the {United_States} or in Europe, to  {P2.the}.  Shipping and insurance charges are at the expense of  {P2.the}, as provided in {Section 6.5}.

Ship.Partial.Sec=<b>Partial Shipments</b>.  If, in the {Order} confirmation, {P1.the} indicates expected dates for shipment that would result in partial shipments,  {P2.the} may choose to hold delivery until shipment is complete or to accept the partial shipments.
...
BuySell.Return.Sec=<b>Return of Goods</b>.  No return of {Products} by  {P2.the} to {P1.the} (“{Returns}&rdquo;) will be authorized or accepted by {P1.the} except as provided in {Section 10.1}.
...
Conf.Def.Def=“{Confidential_Information}&rdquo; shall mean all information disclosed to a {party} (for purposes of this {Section 12}, “{Recipient}&rdquo;) by the other {party} (for purposes of this {Section 12}, “{Discloser}&rdquo;) or its agents or employees in any manner, whether orally, visually or in tangible form (including documents, devices and computer readable media) and all copies thereof, whether created by {Discloser} or {Recipient}.  “{Confidential_Information}&rdquo; shall also include {this_Agreement}, including all provisions, terms and Exhibits.
...
Dispute.ADR.Sec=<b>Dispute Resolution</b>.  In the event of any dispute, claim, question or disagreement (each, a “{Dispute}&rdquo;) arising out of or relating to {this_Agreement}, or the obligations of the {parties} hereunder, the {parties} shall use all reasonable efforts to settle the {Dispute}.  If these efforts are unsuccessful, either {party} may escalate the {Dispute} to their respective chief executive officers by giving a “{Notice_of_Dispute}&rdquo; to the other {party}.  Thereafter, the chief executive officers of the {parties} shall confer promptly and attempt to reach a mutually satisfactory settlement.
